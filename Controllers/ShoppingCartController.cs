...
namespace RedKimchi_API.Controllers
{
    ...
    public class ShoppingCartController : ControllerBase
    {
        ...
        // 데이터 베이스 엑세스를 위한 애플리케이션 유형 컨텍스트 추가
        ...
        {
            // 아래의 명세는 사용자의 의도에 따라 어떤식의 시나리오가 펼쳐질지 적음.
        ...
        public async Task<ActionResult<ApiResponse>> RemoveItemFromCart(string userId, int menuItemId)
        {
            try
            {
                ...
                {
                    _response.IsSuccess = false;
                    _response.ErrorMessages = new List<string> { "쇼핑카트를 찾을 수 없습니다." };
                    _response.StatusCode = HttpStatusCode.NotFound;
                    return NotFound(_response);
                }

                CartItem cartItem = shoppingCart.CartItems.FirstOrDefault(u => u.MenuItemId == menuItemId);
                if (cartItem == null)
                {
                    _response.IsSuccess = false;
                    _response.ErrorMessages = new List<string> { "Item not found in the cart" };
                    _response.StatusCode = HttpStatusCode.NotFound;
                    return NotFound(_response);
                }
